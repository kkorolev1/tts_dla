...
class PositionwiseFeedForward(nn.Module):
    def __init__(self, d_in, d_hid, kernel_sizes, paddings, dropout=0.1):
        ...
        self.sequential = nn.Sequential(
            Transpose(1, 2),
            nn.Conv1d(d_in, d_hid, kernel_size=kernel_sizes[0], padding=paddings[0]),
            nn.ReLU(),
            nn.Conv1d(d_hid, d_in, kernel_size=kernel_sizes[1], padding=paddings[1]),
            Transpose(1, 2),
            nn.Dropout(dropout)
        )
...
class FFT(nn.Module):
    def __init__(self, embed_dim, hid_dim, num_heads,
                 fft_conv1d_kernel, fft_conv1d_padding, dropout=0.1):
        super().__init__()
